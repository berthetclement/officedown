...
* the ability to use the table styles defined in the "reference_docx" which 
serves as a template for the pandoc document.
* the ability to use two list styles defined in the "reference_docx", they are used 
to replace formats of ordered and unordered lists produced in R Markdown.
...
* the replacement of cross-references by cross-references using fields calculated
...
automatic numbering. This is especially important when merging the 
generated word document into a larger document.
...
You can then refer to this caption with bookdown syntax: `\@ref(fig:dotplot)`
...
Their format is defined by a table style located in the template document.
...
all tables. The default table style is to be set in the YAML header of the 
R Markdown document.
...
You can also specify the table style and other tabular options in the knitr chunk 
chunks parameters:
...
> after generating a Microsoft Word document, users have to change 
the style for all list items manually. The styles are already present 
in the reference_docx file used as a template.

The R markdown format `officedown::rdocx_document` is able to change 
unordered and ordered lists produced by pandoc by a default style for each, these 
styles are declared in the YAML header of the R Markdown document. 
...
Some are expected parameters. These parameters need to be 
defined as inline yaml.
